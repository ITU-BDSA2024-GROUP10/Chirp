...
### Unclosed Issues
Some issues still remain open in the Todo column, these are a combination of features that the group found interesting but did not get to implement within the time frame of the project work, bugs, refactors and ducumentation.
&nbsp;
...
There is also some extra functionality left, e.g. a theme song, as well as a feature, for a better display of the users data, and a bug regarding the timestamp on comments. The rest of the issus are regarding refactors for better code quality or in code ducumentation and naming cosistensy through the code.
