...
## Feature: Forget Me
Implementing user data deletion can be complicated in relational databases. Data anonymization can cut ties between records and users but falls short of 'true' deletion since content written by the user remains, which highlights the dichotomy between compliance and intent. Our implementation completely removes user data. User sessions are terminated, all information about the user is deleted; the author is deleted from all the follows-lists they are a part of; and all cheeps the user has written are deleted. While manageable in small systems, this scales with user activity, which poses challenges with performance in large-scale applications. We implemented the feature using Delete On Cascade which automatically removes related records. This simplifies the logic for us, but it also risks unintended data loss. This trade-off helps explain why major platforms often implement it in other ways like anonymization or delayed deletions to balance technical constraints and legal and/or ethical obligations.
